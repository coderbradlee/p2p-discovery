...
var (
	...
	argEnode   = flag.String("boot", "enode://60c59c0c4edb6f809e0751cb9fb22d163d1bfb421cf3f437eb3c78d4ebd250d2a2df5bc3f9cba6b848b08172059d3483a0d8ba6b912ede29da5cd80709258f53@78.138.156.92:10140", "bootstrap node you want to connect to (e.g. enode://e454......08d50@52.176.211.200:16428)")
...
func processArgs() {
	...
	if len(*argIDFile) > 0 {
		...
		if err != nil {
			fmt.Printf("Failed to load file [%s]: %s.", *argIDFile, err)
		}
	}
	...
	if len(*argTopic) > 0 {
		...
		if err != nil {
			fmt.Printf("Failed to parse the topic: %s", err)
		}
	...
	if *asymmetricMode && len(*argPub) > 0 {
		...
		if !isKeyValid(pub) {
			fmt.Printf("invalid public key")
		}
	}

	if len(*argSaveDir) > 0 {
		if _, err := os.Stat(*argSaveDir); os.IsNotExist(err) {
			fmt.Printf("Download directory '%s' does not exist", *argSaveDir)
		}
	} else if *fileExMode {
		fmt.Printf("Parameter 'savedir' is mandatory for file exchange mode")
	}
...
func initialize() {
	...
	if *mailServerMode {
		if len(msPassword) == 0 {
			...
			if err != nil {
				fmt.Printf("Failed to read Mail Server password: %s", err)
			}
		}
	}
	...
	if *argPoW != whisper.DefaultMinimumPoW {
		...
		if err != nil {
			fmt.Printf("Failed to set PoW: %s", err)
		}
	}
	...
	if uint32(*argMaxSize) != whisper.DefaultMaxMessageSize {
		...
		if err != nil {
			fmt.Printf("Failed to set max message size: %s", err)
		}
	}

	asymKeyID, err = shh.NewKeyPair()
	if err != nil {
		fmt.Printf("Failed to generate a new key pair: %s", err)
	}

	asymKey, err = shh.GetPrivateKey(asymKeyID)
	if err != nil {
		fmt.Printf("Failed to retrieve a new key pair: %s", err)
	}
	...
	if err != nil {
		fmt.Printf("crypto/rand failed: %s", err)
	}
...
func configureNode() {
	...
	if *asymmetricMode {
		if len(*argPub) == 0 {
			...
			b := common.FromHex(s)
			if b == nil {
				fmt.Printf("Error: can not convert hexadecimal string")
			}
	...
	if !*asymmetricMode && !*forwarderMode {
		...
		if err != nil {
			fmt.Printf("Failed to create symmetric key: %s", err)
		}
		symKey, err = shh.GetSymKey(symKeyID)
		if err != nil {
			fmt.Printf("Failed to save symmetric key: %s", err)
		}
		if len(*argTopic) == 0 {
	...
	if err != nil {
		fmt.Printf("Failed to install filter: %s", err)
	}
...
func waitForConnection(timeout bool) {
	...
	for !connected {
		...
		if timeout {
			cnt++
			if cnt > 1000 {
				fmt.Printf("Timeout expired, failed to connect")
			}
		}
	}
...
func scanUint(prompt string) uint32 {
	...
	if err != nil {
		fmt.Printf("Fail to parse the lower time limit: %s", err)
	}
	return uint32(i)
}
...
func sendMsg(payload []byte) common.Hash {
	...
	if err != nil {
		fmt.Printf("failed to create new message: %s", err)
	}
...
func messageLoop() {
	...
	if sf == nil {
		fmt.Printf("symmetric filter is not installed")
	}

	af := shh.GetFilter(asymFilterID)
	if af == nil {
		fmt.Printf("asymmetric filter is not installed")
	}
...
func requestExpiredMessagesLoop() {
	...
	if err != nil {
		fmt.Printf("Failed to create symmetric key for mail request: %s", err)
	}
	key, err = shh.GetSymKey(keyID)
	if err != nil {
		fmt.Printf("Failed to save symmetric key for mail request: %s", err)
	}
	...
	for {
		...
		if err != nil {
			fmt.Printf("Wrap failed: %s", err)
		}

		err = shh.RequestHistoricMessages(peerID, env)
		if err != nil {
			fmt.Printf("Failed to send P2P message: %s", err)
		}
...
func extractIDFromEnode(s string) []byte {
	...
	if err != nil {
		fmt.Printf("Failed to parse enode: %s", err)
	}
	return n.ID[:]
}
